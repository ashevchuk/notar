...
Type
  ...
  TStatusProc = procedure(Proc: TCallbackProc); stdcall;

var
	LibPadeg: HMODULE;

function LoadPadeg(UpdateFileExcept: Boolean = False): Integer; stdcall;
...
begin
  if LibPadeg = 0 then
    LibPadeg:=LoadLibrary('padeg_ua.dll');
  if LibPadeg = 0 then
{$IFDEF RAISEERROR}
    Exception.Create('Библиотека padeg_ua.dll не найдена.')
{$ELSE}
    Abort
{$ENDIF}
  else begin
