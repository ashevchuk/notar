...
type
  TatIB_ExternalsLibrary = class(TatScripterLibrary)
...
procedure TatIB_ExternalsLibrary.__InitializeTCTimeStructure(AMachine: TatVirtualMachine);
  var
  Param0Rec: TObject;
...
Param0 := TCTimeStructureWrapper(Param0Rec).ObjToRec;
    IB_Externals.InitializeTCTimeStructure(Param0);
    SetInputArg(0,integer(TCTimeStructureWrapper.Create(Param0)));
  end;
end;

procedure TatIB_ExternalsLibrary.Init;
begin
  Scripter.DefineRecordByRTTI(TypeInfo(TCTimeStructure));
  Scripter.DefineRecordByRTTI(TypeInfo(TM));
  Scripter.DefineRecordByRTTI(TypeInfo(TISC_VARYING));
...
class function TatIB_ExternalsLibrary.LibraryName: string;
begin
  result := 'IB_Externals';
end;

initialization
  RegisterScripterLibrary(TatIB_ExternalsLibrary, True);
